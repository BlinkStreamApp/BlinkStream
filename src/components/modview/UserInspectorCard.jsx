  ...
  const handleSearchSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success && res.value) {
        ...
        setSearchError(`${t('mod.inspector.userNotFound', 'No se encontró el usuario')} @${clean}`)
      }
    } catch {
      setSearchError(`${t('mod.inspector.searchError', 'Error al buscar')} @${clean}`)
    }
      ...
      <div className="h-full flex flex-col bg-[#111119]/90 border border-white/10 rounded-2xl overflow-hidden shadow-xl backdrop-blur-xl">
        <div className="p-3 border-b border-white/10 bg-white/5 flex flex-col gap-2">
          <div className="flex items-center gap-2 text-xs font-bold text-white uppercase tracking-wider">
            ...
            <span>{t('mod.inspector.title', 'Inspector & Buscador de Usuario')}</span>
          </div>
          ...
          <form onSubmit={handleSearchSubmit} className="flex items-center gap-1.5">
            <div className="relative flex-1">
              <input
                ...
                placeholder={t('mod.inspector.placeholder', 'Buscar o escribir @usuario...')}
        ...
        <div className="flex-1 min-h-0 flex flex-col p-2.5 overflow-hidden">
          <div className="flex items-center justify-between px-1 mb-1.5">
            <span className="text-[10px] uppercase font-bold text-text-muted tracking-wider">
              {t('mod.inspector.recent', 'Participantes Recientes')} ({recentChatters.length})
            </span>
            <span className="text-[10px] text-text-muted/60">{t('mod.inspector.clickToMod', 'Clic para moderar')}</span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-1 pr-1">
            {recentChatters.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center p-4 text-center text-text-muted">
                ...
                <p className="text-xs font-semibold text-white/60">{t('mod.inspector.waitingMsgs', 'Esperando mensajes')}</p>
                <p className="text-[11px] text-text-muted/60 mt-0.5">
                  {t('mod.inspector.waitingDesc', 'Los usuarios que hablen en el chat aparecerán aquí para moderación rápida.')}
                </p>
              </div>
            ) : (
              recentChatters.map(chatter => (
                ...
                >
                  ...
                  <span className="text-[10px] font-semibold text-twitch-glow opacity-0 group-hover:opacity-100 transition-opacity shrink-0 ml-2">
                    {t('mod.inspector.inspect', 'Inspeccionar')} →
                  </span>
                </button>
              ))
            )}
    ...
    <div className="h-full flex flex-col bg-[#111119]/95 border border-white/15 rounded-2xl overflow-hidden shadow-2xl backdrop-blur-xl animate-fade-in">
      <div className="p-3 border-b border-white/10 bg-white/5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            title={t('mod.inspector.closeTitle', 'Cerrar Inspector')}
          >
      ...
      <div className="p-3 border-b border-white/10 bg-black/20 space-y-2 shrink-0">
        <div className="flex items-center justify-between">
          <span className="text-[10px] uppercase font-bold text-text-muted tracking-wider">{t('mod.inspector.quickSanctions', 'Sanciones Rápidas')}</span>
          <button
            onClick={() => setShowReasonInput(p => !p)}
            className="text-[10px] text-twitch-glow hover:underline cursor-pointer"
          >
            {showReasonInput ? t('mod.inspector.close', 'Ocultar') : `+ ${t('mod.inspector.customReason', 'Añadir motivo')}`}
          </button>
        </div>
        ...
        {showReasonInput && (
          <input
            type="text"
            placeholder={t('mod.inspector.reasonPlaceholder', 'Motivo de la moderación (opcional)...')}
            value={customReason}
        ...
        <div className="grid grid-cols-4 gap-1.5">
          ...
          <button
            ...
            className="px-2 py-1.5 bg-white/5 hover:bg-amber-500/20 text-amber-300 border border-white/10 hover:border-amber-500/40 rounded-lg text-[11px] font-semibold transition-all cursor-pointer disabled:opacity-50"
          >
            1 {t('mod.quick.followers1h', 'hora')}
          </button>
          <button
            ...
            className="px-2 py-1.5 bg-white/5 hover:bg-amber-500/20 text-amber-300 border border-white/10 hover:border-amber-500/40 rounded-lg text-[11px] font-semibold transition-all cursor-pointer disabled:opacity-50"
          >
            24 {t('mod.quick.followers24h', 'horas')}
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-1.5 pt-0.5">
          <button
            ...
            title={t('mod.inspector.permanentBan', 'Ban Permanente')}
          >
            <PhosphorIcon name="ChatCircleSlash" size={14} weight="bold" />
            <span>{t('mod.action.ban', 'Banear')}</span>
          </button>
          <button
            ...
            title={t('mod.inspector.unban', 'Desbanear')}
          >
            <PhosphorIcon name="CheckCircle" size={14} weight="bold" />
            <span>{t('mod.inspector.unban', 'Desbanear')}</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 flex flex-col p-3 overflow-hidden">
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-[10px] uppercase font-bold text-text-muted tracking-wider">
            {t('mod.inspector.recentMessages', 'Mensajes en esta Sesión')} ({userMessages.length})
          </span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-1.5 pr-1 font-mono text-[11px]">
          {userMessages.length === 0 ? (
            <p className="text-text-muted/50 text-xs italic text-center py-6">
              {t('mod.inspector.noMessages', 'Sin mensajes registrados en esta sesión.')}
            </p>
          ) : (
            userMessages.map(m => (
